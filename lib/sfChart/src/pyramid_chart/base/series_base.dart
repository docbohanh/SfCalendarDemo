...
class _PyramidSeries {
  ...
  void _findVisibleSeries() {
    ...
    chart.series._dataPoints = <PointInfo<dynamic>>[];
    ...
    //Setting seriestype
    currentSeries._seriesType = 'pyramid';
  ...
  void _calculatePyramidEmptyPoints(PyramidSeries<dynamic, dynamic> series) {
    for (int i = 0; i < series._dataPoints.length; i++) {
      if (series._dataPoints[i].y == null)
        series.calculateEmptyPointValue(i, series._dataPoints[i]);
    }
  }
  ...
  void _processDataPoints() {
    for (PyramidSeries<dynamic, dynamic> series in visibleSeries) {
      ...
      _calculatePyramidEmptyPoints(currentSeries);
  ...
  void _calculateVisiblePoints() {
    final List<PointInfo<dynamic>> points = currentSeries._dataPoints;
    chart.series._renderPoints = <PointInfo<dynamic>>[];
  ...
  void _setPointStyle() {
    ...
    final List<PointInfo<dynamic>> points = currentSeries._renderPoints;
    for (int i = 0; i < points.length; i++) {
  ...
  void _findSumOfPoints() {
    ...
    for (PointInfo<dynamic> point in currentSeries._renderPoints) {
      if (point.isVisible) {
  ...
  void _initializeSeriesProperties(PyramidSeries<dynamic, dynamic> series) {
    ...
    if (series.pyramidMode == PyramidMode.linear) {
      _initializeSizeRatio(series, reverse);
    } else {
  ...
  void _initializeSizeRatio(PyramidSeries<dynamic, dynamic> series,
      [bool reverse]) {
    final List<PointInfo<dynamic>> points = series._renderPoints;
    double y;
  ...
  void _pointExplode(num pointIndex) {
    ...
    final PointInfo<dynamic> point = series._renderPoints[pointIndex];
    if (series.explode) {
  ...
  void _calculatePathRegion(num pointIndex) {
    ...
    _calculatePathSegment(currentSeries._seriesType, currentPoint);
  }

  void _calculatePyramidSegments(Canvas canvas, num pointIndex) {
    _calculatePathRegion(pointIndex);
    final dynamic currentPoint = currentSeries._renderPoints[pointIndex];
